...
const searchCustomers = async () => {
    try {
        // 第一引数にリクエスト先のapiのURLとクエリ情報、第二引数に検索結果をcustomers.valueに格納する処理
...
const setCustomer = e => {
   console.log(e)
// 検索欄にクリックした顧客のカナ表記を表示
   search.value = e.kana
// クリックした顧客のidを'update:customerId'をカスタムイベント名として親コンポーネントに打ち上げる
...
<template>
    ...
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
      <div class="modal__container w-2/3" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
        <header class="modal__header">
          ...
          <!-- button type="button"...デフォルトのフォーム送信ボタンとして機能を無効化し、単なるクリックイベントとして実行 -->
                ...
                <tbody>
                  <tr v-for="customer in customers.value.data" :key="customer.id">
                    <td class="border-b-2 border-gray-200 px-4 py-3">
                        <!-- クリックした顧客情報のidとkanaをsetCustomer()の引数に指定 -->
  ...
  <button @click="searchCustomers" type="button" data-micromodal-trigger="modal-1" class="mt-4 flex mx-auto text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">検索する</button>
